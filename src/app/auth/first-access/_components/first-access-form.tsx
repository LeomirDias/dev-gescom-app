...
import {
  firstAccessLookupService,
  firstAccessResendService,
} from "@/modules/authentication/first-access.service"
...
export function FirstAccessForm({
  ...
  const lookupMutation = useMutation({
    mutationFn: firstAccessLookupService,
  })

  const resendMutation = useMutation({
    mutationFn: firstAccessResendService,
  })
  ...
  function goToVerify(form: HTMLFormElement) {
    ...
    router.push(`/auth/first-access/verify?${params.toString()}`)
  }
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="overflow-hidden p-0">
        <CardContent className="grid p-0 md:grid-cols-2">
          <form className="p-6 md:p-8" onSubmit={handleLookup}>
            <FieldGroup>
              ...
              <FieldDescription className="text-center">
                <Link href="/auth/login" className="underline-offset-2 hover:underline underline">
                  Voltar ao login
                </Link>
              </FieldDescription>
            </FieldGroup>
