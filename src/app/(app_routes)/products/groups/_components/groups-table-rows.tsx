...
import { formatDateOnly, formatProfitMargin } from "@/lib/formatters"
...
export function GroupsTableRows({ items, pluralLabel }: GroupsTableRowsProps) {
            ...
            {items.map((item, idx) => (
              ...
              >
                ...
                <td className="px-4 py-3 tabular-nums text-muted-foreground">
                  {formatProfitMargin(item.profitMargin)}
                </td>
        ...
        {items.map((item) => (
          <li key={item.id} className="border bg-card p-4 shadow-sm">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Margem: {formatProfitMargin(item.profitMargin)}
            </p>
